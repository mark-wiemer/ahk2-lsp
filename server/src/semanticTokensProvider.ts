...
function resolve_sem(tk: Token, doc: Lexer) {
    ...
    if ((sem = tk.semantic)) {
        ...
        let r = doc.searchNode(
            l,
            doc.document.positionAt(tk.offset),
            SymbolKind.Variable,
        );
        if (r && r.ref === false) curclass = r.node as ClassNode;
    }
}
...
export async function semanticTokensOnRange(
    ...
    for (let tk of Object.values(doc.tokens)) {
        if (tk.offset < start) continue;
        if (tk.offset > end) break;
        resolve_sem(tk, doc);
    }
    resolve_class_undefined_member(doc), memscache.clear();
    return doc.STB.build();
...
function resolveSemanticType(name: string, tk: Token, doc: Lexer) {
    ...
    switch (sem.type) {
        case SemanticTokenTypes.class:
            ...
            if (curclass?.kind !== SymbolKind.Class) curclass = undefined;
            ...
            if (
                ...
                if (!n || (n as any).def === false) {
                    ...
                    if (t) (n = t), (kind = t.kind);
                        ...
                        if (temp['__CALL']) kind = SymbolKind.Null;
                    } else if (temp['__GET']) kind = SymbolKind.Null;
                }
                switch (kind) {
    ...
    function cls_add_prop(cls: ClassNode, name: string, offset?: number) {
        ...
        if (d && offset) {
            ...
            if (cls.undefined) delete cls.undefined[name];
        } else {
            delete cls.undefined;
            if (d && d.d < 2) (<any>cls).checkmember = false;
            else {
                let t = memscache.get(cls) as any;
                if (t) t['#checkmember'] = false;
            }
        }
    }
}
