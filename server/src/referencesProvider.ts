...
export async function referenceProvider(
    ...
    const refs = getAllReferences(doc, doc.buildContext(params.position));
...
export function getAllReferences(
    ...
    switch (node.kind) {
        case SymbolKind.Field:
            if (scope) {
                const lbs = (<FuncNode>scope).labels;
                if (lbs && lbs[name]) {
        ...
        case SymbolKind.Class:
            if (
                ...
                const all_uris: any = { [doc.uri]: scope };
                if (uri !== doc.uri) {
        ...
        default:
            if (node.kind === SymbolKind.Class || (<FuncNode>node).static) {
                if (node.kind === SymbolKind.Class) {
                    ...
                    const m = (node as FuncNode).full?.match(
                        /^\(([^)]+)\)\sstatic\s([^(]+)($|[(\[])/,
                    );
                    if (!m) {
                        return;
...
function findAllFromDoc(
    ...
    if (kind === SymbolKind.Method || kind === SymbolKind.Property) {
        ...
        if (
            ...
            if (!assume) {
                const local = (<FuncNode>node).local,
