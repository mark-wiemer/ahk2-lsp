...
export async function referenceProvider(
    params: ReferenceParams,
    token: CancellationToken,
): Promise<Location[] | undefined> {
    let result: any = [],
        doc = lexers[params.textDocument.uri.toLowerCase()];
...
export function getAllReferences(
    ...
    context: any,
    allow_builtin = true,
): { [uri: string]: Range[] } | null | undefined {
    ...
    if (!nodes || nodes.length > 1) return undefined;
    let { node, uri, scope, ref } = nodes[0];
    if (!uri || !node.selectionRange.end.character) return undefined;
    ...
    if (ref === true) {
        ...
        for (let it of Object.values(cls.staticdeclaration ?? {}))
            it.children?.length &&
                findAllVar(it as FuncNode, name, range, false, false);
        if (range.length) return { [lexers[uri].document.uri]: range };
        return undefined;
    }

    if (node === lexers[uri].declaration[name]) scope = undefined;
    else if (!scope) scope = doc.searchScopedNode(node.selectionRange.start);
    ...
    switch (node.kind) {
        case SymbolKind.Field:
            if (scope) {
                ...
                if (lbs && lbs[name])
                    references[lexers[uri].document.uri] = lbs[name].map(
                        (it) => it.selectionRange,
                    );
            } else {
                let lbs = doc.labels;
                if (lbs[name])
                    references[doc.document.uri] = lbs[name].map(
                        (it) => it.selectionRange,
                    );
        ...
        case SymbolKind.Class:
            if (
                ...
                !(<any>node).full.includes('.')
            ) {
                if (scope) {
                    if (
                        scope.kind === SymbolKind.Function ||
                        scope.kind === SymbolKind.Method ||
                        scope.kind === SymbolKind.Event
                    ) {
                        if ((<FuncNode>scope).global?.[name]) scope = undefined;
                    }
                }
                let all_uris: any = { [doc.uri]: scope };
                if (uri !== doc.uri) all_uris[uri] = undefined;
                if (!scope)
                    for (const uri in doc.relevance) all_uris[uri] = undefined;
                for (const uri in all_uris) {
                    let rgs = findAllFromDoc(
                        lexers[uri],
                        name,
                        SymbolKind.Variable,
                        all_uris[uri],
                    );
                    if (rgs.length) references[lexers[uri].document.uri] = rgs;
                }
                break;
            }
        default:
            if (node.kind === SymbolKind.Class || (<FuncNode>node).static) {
                if (node.kind === SymbolKind.Class)
                    ...
                    let m = (node as FuncNode).full?.match(
                        /^\(([^)]+)\)\sstatic\s([^(]+)($|[(\[])/,
                    );
                    if (!m) return;
                ...
                for (const uri of new Set([
                    ...
                    refs[lexers[uri].document.uri] = findAllFromDoc(
                        lexers[uri],
                        c[0],
                        SymbolKind.Variable,
                    );
                while (i < l) {
                    ...
                    if (nodes?.[0].node.kind === SymbolKind.Class) {
                        for (let it of Object.values(
                            (nodes[0].node as ClassNode).staticdeclaration ??
                                {},
                        ))
                            it.children?.length &&
                                findAllVar(
                                    it as FuncNode,
                                    'THIS',
                                    (refs[lexers[nodes[0].uri].document.uri] ??=
                                        []),
                                    false,
                                    false,
                                );
                    ...
                    for (const uri in refs) {
                        let rgs = refs[uri],
                            ...
                            arr: Range[] = (references[uri] ??= []);
                        let document = doc.document,
                            tokens = doc.tokens;
                        next_rg: for (let rg of rgs) {
                        ...
                        if (!references[uri].length) delete references[uri];
                    }
                    refs = {};
                }
    ...
    if (Object.keys(references).length) return references;
    return undefined;
}

function findAllFromDoc(
    doc: Lexer,
    name: string,
    kind: SymbolKind,
    ...
    if (kind === SymbolKind.Method || kind === SymbolKind.Property) {
        ...
        if (
            ...
            node.parent?.kind === SymbolKind.Class
        ) {
            ...
            if (!assume) {
                let local = (<FuncNode>node).local,
                    dec = (<FuncNode>node).declaration;
                if (!(local?.[name] || dec?.[name])) assume = undefined;
            } else assume = false;
            node.children?.forEach((it) => {
                if (it.name.toUpperCase() === name)
                    ranges.push(it.selectionRange);
                if (it.children?.length)
                    findAllVar(
                        ...
                        ranges,
                        gg,
                        gg || it.kind === SymbolKind.Function
                            ? assume
                            : undefined,
                        not_static,
                    );
...
function findAllVar(
    ...
    let fn_is_static = node.kind === SymbolKind.Function && node.static,
        f = fn_is_static || node.closure;
    let t: Variable,
        assume = assume_glo;
    if (fn_is_static && not_static && !global) return;
    if (global && node.has_this_param && ['THIS', 'SUPER'].includes(name))
        ...
        node.local?.[name] ||
        ((!f || (global && !assume_glo)) && node.declaration?.[name])
    ) {
        if (!global) return;
        assume_glo = assume = false;
        ...
        if (
            fn_is_static &&
            (!(t = node.declaration?.[name]) ||
                (t.kind === SymbolKind.Variable && !t.def))
        )
            assume = true;
        else if (node.unresolved_vars?.[name]) assume = true;

    if (not_static) not_static = !(<any>node)?.local?.[name]?.static;
    if (assume === global) {
        node.children?.forEach((it) => {
            if (
                it.name.toUpperCase() === name &&
