...
export async function signatureProvider(
    ...
    let uri = params.textDocument.uri.toLowerCase(),
        ...
        nodes: any,
        context: any;
    let signinfo: SignatureHelp = {
        activeSignature: 0,
        signatures: [],
        activeParameter: 0,
    };
    let res = getFuncCallInfo(doc, params.position);
    if (!res || res.index < 0) return undefined;
    ...
    if ((reset_detect_cache(), kind !== SymbolKind.Function)) {
        ...
        if (prop && !allIdentifierChar.test(prop)) return;
        if (kind === SymbolKind.Property) (prop ||= '__item'), (iscall = false);
        ...
        // 	}
        // }
    } else detectExpType(doc, name, pos, ts), (prop = 'call');
    let st = new Set<any>();
    nodes = [];
    for (const tp in ts) {
        let t = ts[tp],
            n: any;
        if (t) add(t.node, t.uri, !/[#@]/.test(tp));
        else if (tp.includes('=>'))
            add((n = ahkvars['FUNC'] as any), n.uri, false);
        else
            for (let t of searchNode(doc, tp, pos, SymbolKind.Variable) ?? [])
        ...
        function add(it: FuncNode, uri: string, isstatic = false) {
            ...
            switch (it.kind) {
                case SymbolKind.Method:
                    if (!iscall) break;
                    if (
                        (needthis = !prop && kind !== SymbolKind.Method) ||
                        !prop
                    ) {
                    ...
                    fn = it as any;
                    if (!(it = ahkvars['FUNC'] as any)) break;
                ...
                if (!iscall)
                    ...
                    if (fn) {
                        if (prop === 'bind') {
                            ...
                            fn.full =
                                '(Func) Bind' +
                                fn.full.slice(fn.full.indexOf('(', 1));
                            n = fn;
                        }
    ...
    nodes.forEach((it: any) => {
        ...
        if ((params = node.params)) {
            ...
            if (overloads.length) {
                let lex = new Lexer(
                    TextDocument.create('', 'ahk2', -10, overloads.join('\n')),
                    undefined,
                    -1,
                );
                    ...
                    if ((params = node.params)) {
                        ...
                        if (needthis)
                            (label =
                                ...
                                parameters.unshift({ label: '@this' });
                        else label = fn + node.full.replace(/^[^(]+/, '');
                        signinfo.signatures.push({
                            label,
                            parameters,
                            documentation:
