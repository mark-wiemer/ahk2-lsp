...
export function symbolProvider(
    ...
    return (symbolcache[uri] = result.map((info) =>
        SymbolInformation.create(
            info.name,
            info.kind,
            info.children ? info.range : info.selectionRange,
            rawuri,
        ),
    ));

    function flatTree(
        node: { children?: DocumentSymbol[]; funccall?: CallInfo[] },
    ...
    function checksamename(doc: Lexer) {
            ...
            if ((dd = lexers[uri])) {
                ...
                checksamenameerr(
                    ...
                    Object.values(dd.declaration).filter(
                        (it) => it.kind !== SymbolKind.Variable,
                    ),
                    dd.diagnostics,
                );
        ...
        function err_extends(doc: Lexer, it: ClassNode, not_exist = true) {
            ...
            doc.diagnostics.push({
                message: not_exist
                    ? diagnostic.unknown("class '" + it.extends) + "'"
                    : diagnostic.unexpected(it.extends),
                range: rg,
    ...
    function converttype(
        ...
        if (
            (tk =
                doc.tokens[
                    (offset = doc.document.offsetAt(it.selectionRange.start))
                ]) &&
            st !== undefined
        ) {
...
export async function workspaceSymbolProvider(
